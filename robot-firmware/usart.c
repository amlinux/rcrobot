...
unsigned char usart_crc_errors = 0;
...
void usart_crc(unsigned char *crc, unsigned char data)
{
    char i;
    for (i = 0; i < 8; i++) {
        if ((*crc ^ data) & 1)
    ...
    switch (usart_rcv_state) {
        ...
        case 1:
            ...
            if (data == 0 || data + unclaimed + 1 >= BUFSIZE - 1) {
                ...
                // accumulating CRC
                usart_crc(&usart_rcv_crc, usart_rcv_len);
            }
            break;
        case 2:
            // accumulating CRC
            usart_crc(&usart_rcv_crc, data);
    ...
    if (usart_rbuf_rptr != usart_rbuf_wbegin) {
        ...
        switch (cmd) {
            ...
            case 0xf1:
                ...
                usart_pkt_put(usart_crc_errors);
